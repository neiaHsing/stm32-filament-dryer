...
6. ESP32 只通过受校验的命令请求运行状态；STM32 保留风扇、传感器、过温联锁和 5 秒远程运行租约。
...
flowchart LR
    ...
    FAN --> MCU
    MCU -->|"SPI 半双工：状态"| ESP["ESP32 局域网网关"]
    ESP -->|"同一数据线：控制"| MCU
```
...
- PA0、PA2 已分配给电机驱动，PA3 可用于安全继电器，PB13～PB15 用于 ESP32-S 半双工状态和控制通信，PA9、PA10 保持预留。
...
原有功能引脚保持不变，电机引脚已经在固件中启用，ESP32 控制复用 PB15 半双工数据线，安全链引脚仍为规划项：
...
如果正转方向与机械结构要求相反，可交换两根电机线，或在软件中交换 IN1、IN2 的控制逻辑。PA9、PA10 均保持悬空并标记 NC，不要在这两个引脚上安装 JGB37-520 编码器插座、编码器上拉或电平转换电路。
...
| J11 ESP32 数据与控制 | JST-XH 4P | 1:PB13/SCK→GPIO18，2:PB14/CS→GPIO27，3:PB15/SDIO↔GPIO23，4:GND；不向 ESP32 供电 |

PTC 主电流不得使用 JST-XH 或普通小型 KF301 克隆端子。PTC 支路使用额定 15A 以上的压接端子、保险丝座和线束。

ESP32 接口只用 J11 四根线。STM32 每秒先通过 PB15→GPIO23 发送固定 64 字节 CRC 状态帧，随后双方换向，再由 GPIO23→PB15 返回固定 64 字节命令；没有待发命令时返回带 CRC16 的 `N*48FA\r\n`，其余字节补零。PA9、PA10 和 GPIO17 不参与通信。远程运行期间 ESP32 每秒续租，STM32 连续 5 秒未收到续租会关闭 PTC 和电机并停止任务。CRC16、会话号、序号及风扇、传感器、过温联锁均保持启用。

PB15 与 GPIO23 建议串联 330 Ω～1 kΩ 电阻，限制复位或换向异常时的争用电流。现有直连可先在任务停止、PTC 与电机断开的条件下验证。经典 ESP32 使用 GPIO matrix workaround，在同一 GPIO 上分别路由 SPI 从机输入与输出，并只在 64 字节下行时隙打开输出；这不是 ESP-IDF 公共 API 提供的通用三线从机模式。网页请求需要等下一个一秒时隙，确认再随下一次状态返回，因此最终确认通常约有 1～2 秒延迟。
...
9. J11 的 PB15/SDIO 走线应短且远离 PTC、电机和继电器节点；PB15 与 GPIO23 间优先预留 330 Ω～1 kΩ 串联电阻，J11 的 GND 提供共同参考。
...
| 远程停止与设置 | PTC 断开时通过网页/API 提交 `running=false` 和新目标 | STM32 确认应用，实际目标回读一致，PTC 和电机保持关闭 |
| 远程租约 | 远程任务中断开 PB15/SDIO 或停止 ESP32 心跳 | 最迟 5 秒后任务停止，PTC 和电机关闭，状态返回租约到期 |
