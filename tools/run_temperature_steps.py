#!/usr/bin/env python3
"""Run bounded sequential temperature steps at 24 C with distinct-sample stability gates."""
...
def parse_args():
    ...
    parser.add_argument("--targets", type=int, nargs="+", default=[50,52])
...
def main():
    ...
    def run_to_target(target, phase_name):
        ...
        while True:
            ...
            now = time.monotonic()
            if data['stm32_tick_ms'] != window_tick:
                window_tick=data['stm32_tick_ms']
                window.append((window_tick/1000,temperature,float(data['output_permille'])))
            device_now=window_tick/1000
            window=[v for v in window if device_now-v[0]<=120]
            first=[v for v in window if v[0]<device_now-60]
            last=[v for v in window if v[0]>=device_now-60]
            stable=(len(window)>110 and first and last and
                    max(v[1] for v in window)-min(v[1] for v in window)<=0.3 and
                    abs(statistics.mean(v[1] for v in last)-statistics.mean(v[1] for v in first))<0.05 and
                    abs(statistics.mean(v[2] for v in last)-statistics.mean(v[2] for v in first))<10)
    ...
    try:
        ...
        for target in args.targets:
            if not 30 <= target <= 65: raise ValueError('target out of experiment range')
            _, _, _, data = run_to_target(target, 'hold_' + str(target))
            result['stages'].append(data)
        result['final_status'] = stop_and_confirm(args.targets[-1])
