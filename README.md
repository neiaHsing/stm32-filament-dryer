...
- 旋钮菜单 `ROOM TEMP CAL` 手动输入当前室温（−10～50°C，1°C 步进），并把该值保存和上报给网关；当前主线 PID 输出不把室温作为加法前馈项。
...
固件启动默认参数为 `Kp=160`、`Ki=2`、`Kd=180`、积分分离阈值 `2°C`，输出单位为千分比（`1000 = 100%`）。ESP32 的 `/api/pid` 可以在运行时提交 Kp、Ki、Kd，范围分别为 0～500、0～50、0～1000；参数只保存在 RAM，STM32 重启后恢复默认值。
...
2. 下降扰动冻结计时已结束；
3. `abs(v) <= 0.10°C/s`，温度变化已经较慢；
4. 当前欠温但预测值已经到温时，不继续累加正积分。

候选积分为：

```text
Icandidate = clip(I + Ki × g × e × dt, -250, 250)
```
...
```text
u = clip(P + I + D + B, 0, 1000)
```

这里没有加法式室温前馈项。`ROOM TEMP CAL` 的值目前用于 W25Q 保存、SPI 遥测和网关显示；历史算法报告中的 `F22`、`20.13` 等前馈公式属于实验版本说明，不是当前 `SeparationPid::step()` 的输出路径。主机测试也明确验证了目标等于当前温度时不会凭空产生保持功率。
...
室温是手动输入值，不会自动跟随环境变化，也不改变箱内传感器示值。当前主线把它作为保存和上报字段，未将其加入 PID 输出；未连接可用 W25Q 时设置仅在本次通电期间生效。
