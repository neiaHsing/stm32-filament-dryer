...
#define OLED_ADDRESS_3C (0x3CU << 1U)
#define OLED_ADDRESS_3D (0x3DU << 1U)
#define OLED_TIMEOUT  100U
static I2C_HandleTypeDef *g_pHI2COLED = &hi2c1;
static uint16_t g_oledAddress = OLED_ADDRESS_3C;
...
{
    if (!g_oledReady) return HAL_ERROR;
    uint8_t tmpbuf[2];
    ...
    HAL_StatusTypeDef status = HAL_I2C_Master_Transmit(
        g_pHI2COLED, g_oledAddress, tmpbuf, 2, OLED_TIMEOUT);
    if (status != HAL_OK) g_oledReady = false;
    return status;
}
...
static int OLED_WriteNBytes(uint8_t *buf, uint16_t length)
{
    ...
    HAL_StatusTypeDef status = HAL_I2C_Mem_Write(
        g_pHI2COLED, g_oledAddress, 0x40, I2C_MEMADD_SIZE_8BIT,
        buf, length, OLED_TIMEOUT);
    if (status != HAL_OK) g_oledReady = false;
    return status;
}
...
{
    static const uint16_t addresses[] = {OLED_ADDRESS_3C, OLED_ADDRESS_3D};
    g_oledReady = false;
    HAL_Delay(20U);

    for (uint8_t i = 0U; i < 2U; ++i)
    {
        if (HAL_I2C_IsDeviceReady(g_pHI2COLED, addresses[i], 3U, 50U) == HAL_OK)
        {
            g_oledAddress = addresses[i];
            OLED_Init();
            if (g_oledReady)
            {
                OLED_Clear();
