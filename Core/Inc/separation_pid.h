...
struct SeparationPid {
  // Output units are permille. Integral is the residual around feedforward.
  static constexpr float feedforwardSlopePermillePerC = 11.99435f;
  static constexpr float feedforwardOffsetC = 12.83620f;
  ...
  static float holdingPower(float target,
                            float ambient = AMBIENT_TEMPERATURE_DEFAULT_C) {
    // Two holding-duty points at 24 C, normalized to 22 C (2026-09-17).
    // See outputs/feedforward_calibration_24c/metrics.json.
    // Scale by the target-to-ambient
    // temperature difference so a colder room receives more holding power.
    const float referenceDelta = target - AMBIENT_TEMPERATURE_DEFAULT_C;
    if (referenceDelta <= 0 || ambient >= target) return 0;
    const float baseline = clamp(
        feedforwardSlopePermillePerC * (target - feedforwardOffsetC),
        0, 850);
    return clamp(baseline * (target - ambient) / referenceDelta, 0, 850);
  }
  void reset() {
  ...
  float step(float target, float measured, float dt, float kp, float ki,
    ...
    const float feedforward = holdingPower(target, ambient);
    ...
    const float coolingBoost = error > -0.20f && error <= 2.0f && derivative < 0
        ? clamp(-derivative * (1000.0f - feedforward) / rate, 0, 100)
        : 0;
    ...
    // Integral correction is allowed throughout the separation band. During a
    // rising approach, reduce its rate instead of freezing it at a fixed
    // slope threshold. This lets the controller build the holding correction
    // while it is still climbing from 38 to 40 C, without banking the full
    // correction as quickly as it would while stationary.
    if (fabsf(error) <= separation && recoveryHold == 0 &&
        !(error > 0 && predictedError <= 0)) {
      const float risingIntegralScale =
          error > 0 && derivative > 0
              ? clamp(1.0f - derivative / 0.30f, 0.25f, 1.0f)
              : 1.0f;
      const float candidate = clamp(
          integral + ki * gain * error * dt * risingIntegralScale,
          -250, 250);
      const float raw = feedforward + p + candidate + d + coolingBoost;
    ...
    ready = true;
    previousOutput = clamp(feedforward + p + integral + d + coolingBoost,
                           0, ceiling);
